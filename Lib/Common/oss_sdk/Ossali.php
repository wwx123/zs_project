...
class Ossali{
	...
	/**
	 * OSS服务地址
OSS API 杭州节点内网通信地址：oss-internal.aliyuncs.com
OSS API 杭州节点 外网通信地址：oss.aliyuncs.com
OSS API 青岛节点外网通信地址： oss-cn-qingdao.aliyuncs.com
OSS API 青岛节点内网网通信地址：oss-cn-qingdao-internal.aliyuncs.com
	 */
// 	const DEFAULT_OSS_HOST = 'oss.aliyuncs.com';//默认使用杭州服务器外网api
	...
	const OSS_PREFIX = 'prefix';                                               //前缀
	const OSS_DELIMITER = 'delimiter';                                         //分隔符
	const OSS_MARKER = 'marker';                                               //分页开始位置
	const OSS_CONTENT_MD5 = 'Content-Md5';                                     //上传内容的md5值
	const OSS_CONTENT_TYPE = 'Content-Type';                                   //上传数据的格式
	const OSS_CONTENT_LENGTH = 'Content-Length';                               //上传数据的长度
	const OSS_IF_MODIFIED_SINCE = 'If-Modified-Since';                         //文件最后修改时间，对比文件是否被修改
	...
	const OSS_CONTENT = 'content';                                              //请求上传的内容如xml数据
	const OSS_BODY = 'body';
	const OSS_LENGTH = 'length';
	const OSS_HOST = 'Host';                                                   //主机请求地址
	const OSS_DATE = 'Date';                                                   //请求时间
	const OSS_AUTHORIZATION = 'Authorization';                                 //授权签名
	const OSS_FILE_DOWNLOAD = 'fileDownload';
	const OSS_FILE_UPLOAD = 'fileUpload';
	const OSS_PART_SIZE = 'partSize';
	const OSS_SEEK_TO = 'seekTo';
	const OSS_SIZE = 'size';
	const OSS_QUERY_STRING = 'query_string';                                   //请求的参数 & 号后面的
	const OSS_SUB_RESOURCE = 'sub_resource';                                   //请求的资源 ? 号后面的
	...
	public function auth($options){
		...
		$msg .= "<br/><br/>请求头 HEADERS:----------------------------------------------\n".serialize($request->request_headers)."\n";
		
		$request->send_request();//发送请求

		$response_header = $request->get_response_header();//获取相应头数据
		...
		$msg .= "<br/><br/>响应数据 DATA:----------------------------------------------\n".serialize($data)."\n";
		$msg .= date('Y-m-d H:i:s').":<br/>---LOG END---------------------------------------------------------------------------\n";
		//add log
		$this->log($msg);
	...
	public function create_bucket($bucket, $acl = self::OSS_ACL_TYPE_PRIVATE, $options = NULL){
		...
		$xml->addChild('LocationConstraint','oss-cn-qingdao');
