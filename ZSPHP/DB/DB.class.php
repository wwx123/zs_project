...
class DB {
	...
	public function __construct($host, $user, $password, $dbname , $connect = false, $charset = 'utf8mb4') {
		$this->host = $host;
		$this->user = $user;
	 ...
	 * @param boolean $replace 是否replace
	 */
	public function inserts ($table, $set = array(),$replace=false) {
		...
		foreach($set as $val){
			...
			foreach ($val as $key=>$value) {
				$fields[] = "`{$key}`";
				$valuesData[] = $this->parseValue($value);
			}
